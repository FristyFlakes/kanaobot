...
from kanaorobot.utils import text_shortner
...
@KANAO.on_message(filters.command(["malid"], prefixes = "/") & ~filters.edited)
async def mal(client, message):
    ...
    buttons = [
                [InlineKeyboardButton("malanime", f"anime {query[-1]}"), InlineKeyboardButton("malmanga", f"manga {query[-1]}"), InlineKeyboardButton("malchar", f"char {query[-1]}")]
              ]
