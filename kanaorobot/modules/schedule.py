...
@KANAO.on_message(filters.command(["schedule"], prefixes = "/") & ~filters.edited)
async def schedule(client, message):
    query = message.text.split()
    if len(query) < 2:
        text = "You forgot to mention day!\nExample:\n**/schedule monday**"
        await KANAO.send_message(chat_id = message.chat.id, text = text, parse_mode = "markdown")
        return
    try:
        ...
        await KANAO.send_message(chat_id = message.chat.id, text = SCHEDULE_TEXT, parse_mode = "markdown")
